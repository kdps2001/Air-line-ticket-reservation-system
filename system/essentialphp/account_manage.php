<?php
    ...
    if(!isset($_POST['manege_user_id']) && !isset($_POST['submit']))
    {
        header('Location: ../dashboard.php?error=notgetvalues');
        exit();
    }

    $manege_user_id = $conn->real_escape_string($_POST['manege_user_id']);
    $operation = $_POST['submit'];

    if($operation == 'suspend')
    {
        $status_up = " UPDATE user SET user_status = 'suspend' WHERE user_id = '$manege_user_id'";
    ...
    {    
        $query = "SELECT * FROM user WHERE user_id = '$manege_user_id'";
        ...
        if($result)
        {
            $delete_query = "DELETE FROM user WHERE user_id = '$manege_user_id'";
